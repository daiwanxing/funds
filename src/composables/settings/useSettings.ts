...
export function useSettings() {
  ...
  const sortTypeObj = ref<SettingsState['sortTypeObj']>({ name: null, type: null })
  ...
  function load(): Promise<void> {
    return new Promise((resolve) => {
      storage.get(SETTINGS_KEYS as unknown as string[], (res: Record<string, any>) => {
        ...
        if (res.fundListM) {
          ...
          fundListM.value = fundList.map((code: string) => ({ code, num: 0 }))
