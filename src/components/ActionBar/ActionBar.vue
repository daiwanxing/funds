...
<template>
  <div class="flex flex-wrap gap-1 py-1 items-center">
    ...
    <button class="btn text-blue-500 border-blue-500" @click="$emit('reward')">打赏</button>
  </div>
  ...
  <div v-if="showCost || showGains" class="flex flex-wrap gap-1 py-1">
    <button
      v-if="showGains"
      class="btn text-xs"
      :class="allGains[0] >= 0 ? 'text-red-500 border-red-300' : 'text-green-600 border-green-300'"
    >
    ...
    <button
      v-if="showCost"
      class="btn text-xs"
      :class="allCostGains[0] >= 0 ? 'text-red-500 border-red-300' : 'text-green-600 border-green-300'"
    >
  ...
  <!-- Refresh button -->
  <div class="absolute right-2 bottom-2 cursor-pointer" @click="$emit('refresh')">
