...
<template>
  <div v-if="visible" class="fixed inset-0 z-1001 bg-black/70 flex items-start justify-center pt-5">
    <div class="bg-white rounded-15px w-500px text-center" :style="{ marginTop: top + 'px' }">
      <div class="py-3">
        <button
      ...
      <div v-if="checked === 'export'" class="px-4">
        <el-input type="textarea" :rows="15" v-model="exportConfigStr" placeholder="配置内容" />
        <button class="text-xs border border-green-500 text-green-600 rounded px-3 py-1.5 cursor-pointer bg-white mt-3" @click="copyToClipboard">复制到剪贴板</button>
      </div>
      <div v-else class="px-4">
        <el-input type="textarea" :rows="15" v-model="inputConfigStr" placeholder="请在此输入框粘贴配置文本" />
        <button class="text-xs border border-green-500 text-green-600 rounded px-3 py-1.5 cursor-pointer bg-white mt-3" @click="importInput">提交配置文本</button>
      </div>
      <div class="py-3">
        <button class="text-xs border border-gray-300 rounded px-3 py-1.5 cursor-pointer bg-white" @click="close">返回</button>
      </div>
    </div>
  </div>
</template>
