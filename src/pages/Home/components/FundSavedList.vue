...
<template>
  <div class="flex-1 flex flex-col h-full overflow-hidden bg-[#161618]">
    ...
    <!-- Body -->
    <ul v-if="fundData.dataList.value.length > 0" class="flex-1 overflow-y-auto pb-10">
      <li 
      ...
      >
        ...
        <div v-if="item.fundcode === activeCode" class="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-[14px] bg-[#3B82F6] rounded-r shadow-[0_0_8px_rgba(59,130,246,0.6)]"></div>
        ...
        <div class="flex items-start gap-[6px] shrink-0 font-mono tracking-tight pt-0.5">
           ...
           <span class="w-14 flex justify-end">
              <span class="px-1.5 py-[1px] rounded text-[11px] font-semibold tracking-tighter shadow-sm" :class="Number(item.gszzl) >= 0 ? 'bg-up/10 text-up' : 'bg-down/10 text-down'">
    ...
    <!-- Empty State -->
    <div v-else class="flex-1 flex flex-col items-center justify-center pb-20">
      <div class="w-16 h-16 rounded-2xl bg-white/[0.03] border border-white/5 flex items-center justify-center mb-5">
         <span class="text-2xl">📊</span>
      </div>
      <p class="text-[14px] font-bold text-white mb-2 font-sans tracking-wide">添加你的第一只基金</p>
      <p class="text-[12px] text-white/40 text-center mx-6 leading-relaxed font-sans">从上方搜索框输入基金代码或缩写，快速加入自选列表。</p>
    </div>
  </div>
</template>
