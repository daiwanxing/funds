...
> 即使在无自选基金的空状态下，总体网格结构依然保持固定对立（左侧 `380px`，右侧自适应）。Zone B 将在列表区域内聚呈现空状态图标引导（Star + Plus），且底部状态栏会自动隐去为「0只」的提醒数量。
...
| 断点 | 变化 |
|---|---|
| **≥1000px** | 完整两列布局（Zone B 固定 380px + Zone C 自适应撑满），FAB 正常可见 |
...
| 4 | Zone B 列 | **固定 380px**，仅核心指标展示（精简列）|
